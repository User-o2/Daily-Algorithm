...
Upd 补充：
search函数的作用其实就是让alls和a[]这俩数组实现共享索引（a[]数组有一个单位的偏移）
alls存储的是排序且去重后的真实数轴位置x，意义在于反映相对大小顺序。例如，alls[0]就是全场出现的最小坐标，alls[1]就是第二小坐标
a[]存储的是数轴位置x上被累加上的值，a[1]存的就是最小坐标上的值，a[2]存的就是第二小坐标上的值。
*/
